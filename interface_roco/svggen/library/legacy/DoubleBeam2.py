...
class Beam(Component):
  ...
  def assemble(self):
    ...
    length2 = self.getParameter("length2")
    ...
    faces = []
    for i in range(len(thetas)):
      faces.append(Face('', ((thickness, dr[i]), (thickness, length2 - dl[i]), (0, length2 - dl[i-1]), (0, dr[i-1]))))
    for i in range(phase):
      faces.append(faces.pop(0))
    ...
    for i in range(len(faces)):
      i = i + len(faces)
      if i == len(faces):
        fromEdge = "r%d.e0" % (len(faces) - 1)
        j = Joint("REVOLUTE", -2*pi, 2*pi)
        graph.attachFace(fromEdge, faces[i - len(faces)], 'e0', prefix="r%d"%i, angle = -angle, edgeType="JOINT", joints=[j])
      
      else:
        graph.attachFace(fromEdge, faces[i - len(faces)], 'e3', prefix="r%d"%i, angle = angle)
      fromEdge = "r%d.e1" % i
...
if __name__ == "__main__":
  ...
  b.setParameter("length2", 100)
  b.setParameter("beamwidth", 10)
  b.setParameter("shape", 4)
  b.setParameter("tangle", 90)
  b.setParameter("bangle", 90)
  b.setParameter("phase", 2)
  
  b.sympyicize("length")
  b.sympyicize("length2")
  b.sympyicize("beamwidth")

  b.makeOutput("output/DoubleBeam2", protobuf=True, display=False)
